...
const PLAN_LABEL: Record<string, string> = {
  version_1: 'Version 1 Plan',
  version_2: 'Version 2 Plan',
  version_3: 'Version 3 Plan',
};

/**
 * Read-only billing history for the signed-in buyer — mirrors
 * vendor/billing-history.tsx exactly, scoped to the buyer's own account
 * (resolved server-side from the session email, same as
 * createBuyerPaymentLink already does) instead of a vendor record.
 */
export default function BuyerBillingHistory() {
  const { activeBuyerAccount, showToast } = useApp();
  const [links, setLinks] = useState<PaymentLinkRecord[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [downloadingId, setDownloadingId] = useState<string | null>(null);

  const buyerAccountId = activeBuyerAccount?.id;
  ...
  const handleDownload = async (link: PaymentLinkRecord) => {
    ...
    try {
      ...
      const res = await fetch(
        `/api/buyer-accounts/${encodeURIComponent(buyerAccountId)}/payment-links/${encodeURIComponent(link.id)}/invoice`,
        { headers: token ? { Authorization: `Bearer ${token}` } : {} }
      );
  ...
  if (!buyerAccountId) {
    return (
      <div className="max-w-3xl mx-auto p-8 text-center rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-gray-900/80">
        ...
        <p className="text-sm text-slate-600 dark:text-gray-300">Could not find your buyer account.</p>
      </div>
    );
  }
