describe('Database migration (Neon PostgreSQL)', () => {
  ...
  describe('runCli', () => {
    ...
    test('warns and exits 0 when cloud database provider data transfer quota is exceeded', async () => {
      let freshMigrate;
      jest.isolateModules(() => {
        jest.doMock('../src/db/pool', () => ({
          pool: {},
          query: jest.fn().mockRejectedValue(new Error('Your project has exceeded the data transfer quota. Upgrade your plan to increase limits.')),
        }));
        freshMigrate = require('../src/db/migrate');
      });
      jest.spyOn(console, 'log').mockImplementation(() => {});
      ...
      expect(warnSpy).toHaveBeenCalledWith(
        '[migrate] Warning: Cloud database provider quota exceeded:',
        expect.stringContaining('data transfer quota')
      );
      expect(exitSpy).toHaveBeenCalledWith(0);
    });
  });
