...
function runCli() {
  return migrate()
    ...
    .catch((err) => {
      if (err && err.message && /data transfer quota/i.test(err.message)) {
        console.warn('[migrate] Warning: Cloud database provider quota exceeded:', err.message);
        console.warn('[migrate] Operating in fallback mode. Schema verification bypassed due to external cloud quota limits.');
        process.exit(0);
        return;
      }
