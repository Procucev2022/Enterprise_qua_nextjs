...
describe('Vendor Item SKU Catalogue API', () => {
  ...
  test('POST /api/catalogue creates new SKU product', async () => {
    ...
    const res = await request(app).post('/api/catalogue').send(prod);
  ...
  test('PUT /api/catalogue/:id returns 404 for invalid product', async () => {
    const res = await request(app).put('/api/catalogue/invalid-prod-id').send({});
    expect(res.statusCode).toBe(404);
  });

  test('DELETE /api/catalogue/:id removes product', async () => {
    const res = await request(app).delete(`/api/catalogue/${createdProdId}`);
    expect(res.statusCode).toBe(200);
    expect(res.body.success).toBe(true);

    const failRes = await request(app).delete('/api/catalogue/invalid-prod-id');
