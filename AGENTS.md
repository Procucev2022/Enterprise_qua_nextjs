...
- Every single application code file in `app/` and `lib/` **MUST** achieve at least **90% unit test coverage** individually across all four metrics:
  - **Statements: >= 90%**
  - **Branches: >= 90%**
  - **Functions: >= 90%**
  - **Lines: >= 90%**
- **Zero file exclusions**: Do not skip or exclude any application file from testing or coverage thresholds.
- If even a single file falls below 90% in any of the four parameters, the test run fails and must be corrected before finishing the task.

## 2. Automated Test Verification on Every Change
Whenever making any code changes or adding new features:
1. Immediately write or update corresponding unit tests in `__tests__/`.
...
- Place test files in `__tests__/`, mirroring the project source directory structure (e.g. `__tests__/lib/store.test.tsx`, `__tests__/app/components/Header.test.tsx`, `__tests__/app/api/rfqs.test.ts`).
